...
const Update = ({ setOpration }) => {
  ...
  const { formData, setFormData, handleUpdate } = useContext(ProductContext);
  const [isValidate, setValidate] = useState(false);
  
  const [productImg, setProductImg] = useState(formData.img);
  ...
  const OnSubmit = () => {
    setOpration("view");
    handleUpdate({ ...formData, img: productImg });
  };
